...
import { User } from '@/lib/store/zustand'
      ...
      {users.map(user => (
        ...
        >
          ...
          <div className="ml-3 overflow-hidden">
            <p className={`font-medium text-sm truncate ${selectedUserId === user.id ? 'text-purple-700 dark:text-purple-200' : 'text-gray-800 dark:text-gray-200'}`}>{user.full_name}</p>
            <p className={`text-xs truncate ${selectedUserId === user.id ? 'text-purple-500 dark:text-purple-400' : 'text-gray-500 dark:text-gray-400'}`}>{/*user.lastMessage*/}lastMessage</p>
          </div>
          {selectedUserId === user.id && (
